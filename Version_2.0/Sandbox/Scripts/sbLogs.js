...
function showFullDialog() {
    ...
    logs.forEach(log => {
        ...
        if (log.direction === 'sent') { 
            backgroundColor = changeBackgroundColor(ovon.sender?.to);
            if (inviteEvent) {
                ...
                if (whisperEvent) {
                    ...
                    logElement.textContent = `${user} - Bare Invite to: ${localStorage.getItem("assistantName")} at ${ovon.sender.to || 'unknown'}`;
                }
            }else {
...
function displayLogs(logsToDisplay) {
    ...
    logsToDisplay.forEach((log, index) => {
        ...
        if (logDirection === 'sent' || logDirection === 'received') {
            ...
            const ovon = contentObject?.ovon || {};
            senderTo = ovon.sender?.to;
            senderToList.push(senderTo);
            senderToList = senderToList.filter(value => value!== 'Human' && value !== undefined && value !==localStorage.getItem('humanFirstName'));
...
function displayLogFileContent(content, filter) {
    ...
    logEntries.forEach(logEntry => {
        ...
        if (logLine.ovon?.events && logLine.ovon.events.length >= 0) {
            ...
            if (firstEvent.sender.to) {
                sentTo = firstEvent.sender.to;
                console.log(sentTo);
                sentList.push(sentTo);
                sentList = sentList.filter(value => value !== 'Human' && value !== undefined && value !==localStorage.getItem('humanFirstName'));
                    ...
                    if (isInviteSent) {
                        ...
                        logHeader.appendChild(document.createTextNode(
                            whisperEvent
                                ...
                                : ` Bare Invite to: ${logLine.ovon.sender.to || 'unknown'}`
                        ));
                    } else {
                        const sentLabel = document.createElement('span');
                        sentLabel.textContent = 'Sent: ';
