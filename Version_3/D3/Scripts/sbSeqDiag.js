...
function buildManifestReqSeqDiag(wc, aName ) {
  buildSeqDiagJSON( "human", wc, "[Request Manifest to "+aName+"]", "Basic Manifest Request", "" );
  buildSeqDiagJSON( wc, wc, "[REQUEST]", "Initiate for a manifest request", "" );
  buildSeqDiagJSON( wc, aName, "request manifest "+aName, "actual manifest request", "" );
  buildSeqDiagJSON( aName, aName, "[BUILD MANIFEST REPORT]", "Compose a manifest object for the request", "" );
  saveLocalSeqDiag();
}
...
function buildWhisperInviteReqSeqDiag(wc, assistantName, whispCMD) {
  buildSeqDiagJSON( assistantName, wc, whispCMD, "Internal propietary command for the platform", "" );
  saveLocalSeqDiag();
}
